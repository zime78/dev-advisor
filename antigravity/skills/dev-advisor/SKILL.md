---
name: dev-advisor
description: 6중 카탈로그(547 패턴 / 273 알고리즘 / 75 언어 / 106 보안 / 214 원칙 / 20 품질 + 18 부록)로 앱 개발을 안내하는 어드바이저. 9 advisor 모드 (7 기본 + 2 통합) — recommend / validate / refactor / maintain / security-audit / qa / qc / full / swarm + lookup.
---
...
| **Principles** | `references/principles/` | **214 + 18 부록** | **SOLID / GRASP / ISO 25010 / 12-Factor / Code Smells + 타입 시스템 / 동시성 이론 / Refactoring 기법 / SW 경제·추정 / 진화적 아키텍처 / 탄력성 / 문서화 / DORA / 성능 / Green Software + DB 기초 / SDLC 모델 / Scaled Agile / 직업 윤리 + Standards Mapping (SWEBOK/CS2023/DMBOK/OWASP/NIST-ISO) / Configuration Management (IEEE 828 SCMP) + HCI Methodology (ISO 9241 / Persona / Journey Map / Heuristic Eval) / Formal Methods (TLA+ / Alloy / Hoare / Model Checking)** | 23 파일 + index + 부록 [`micro-principles.md`](references/principles/micro-principles.md) (DRY/KISS/YAGNI/LoD/SoC/Tell-Don't-Ask/Composition/SSoT + Conway/Hyrum/Postel/Brooks/Hollywood/Boy Scout/Pareto/Goodhart/Cunningham/Inverse Conway, verify 카운트 외, `/principle <id>` 호출 가능). advisor 9 모드의 핵심 표준 인용 source |
...
합계 **1,235 항목 + 18 부록** (verify 카운트 기준).
...
| 명령        | 패턴 (547)           | 알고리즘 (273)                  | 언어 (75)          | 보안 (106)             | 원칙 (214 + 18 부록)   | 품질 (20) |
|-------------|---------------------|--------------------------------|--------------------|-----------------------|-----------------------|-----------|
...
- **list**: JSON item 목록을 기준으로 도메인 카테고리 진입점 표 응답. 패턴/보안/원칙/품질은 `<domain>/index.md` 카테고리 매핑, 알고리즘은 `references/algorithms/index.md` 32 카테고리 표, 언어는 `references/languages/index.md` 우선순위 + 분야 매트릭스. **list <category>**: 카테고리 파일 `## 목차` 응답 (알고리즘만 카테고리 인자).
...
카탈로그 lookup (기존 보존):
  ...
  /principle <id|list|search>             214 + 18 부록 SW 공학 원칙
...
- 원칙 (214 + 18 부록) — [`references/principles/index.md`](references/principles/index.md) · 미시 부록: [`micro-principles.md`](references/principles/micro-principles.md)
