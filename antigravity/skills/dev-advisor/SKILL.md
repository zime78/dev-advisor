---
name: dev-advisor
description: 6중 카탈로그(543 패턴 / 292 알고리즘 / 75 언어 / 106 보안 / 211 원칙 / 20 품질 + 20 부록)로 앱 개발을 안내하는 어드바이저. 9 advisor 모드 (7 기본 + 2 통합) — recommend / validate / refactor / maintain / security-audit / qa / qc / full / swarm + lookup.
---
...
- **[필수]** Antigravity에서 이 스킬을 실행할 때는 `references/`와 `scripts/`를 현재 스킬 디렉터리 기준 상대 경로로 읽는다. 사용자 홈의 Codex 경로 또는 다른 런타임 경로를 하드코딩하지 않는다.
- **[필수]** `swarm` 또는 hand-off에서 분석·설계·리뷰 에이전트를 호출할 때는 `invoke_subagent(Subagents=[{"TypeName": "<name>", "Role": "<name> Analyst", "Prompt": "..."}])` 형식을 사용한다.
...
| 9 | **병렬 심층 점검 (Swarm)** | `swarm`          | 7 Antigravity 서브에이전트 병렬 결과 + reviewer 통합 + Top 10 |
...
| 도메인 | 디렉토리 | 항목 수 | 용도 | 구조 |
|---|---|--:|---|---|
| Patterns   | `references/patterns/`   | 543 | 디자인·아키텍처 의사결정 (산업 도메인 포함, MDM·DQ 거버넌스 + Web Performance / Data Warehousing & BI / HPC-Scientific 신설) | 55 카테고리, `patterns/<category>.md` + `patterns/domains/<industry>.md` 평면 inline + 카테고리당 `## N.` 헤더 |
| Algorithms | `references/algorithms/` | 292 | 자료구조·연산·분산·동시성·파싱·DB·OS·미디어 | 33 카테고리, `<a id>` anchor + 카테고리 목차 + `algorithms/index.md` 전역 ID 매핑 (4중 lookup) |
...
| **Principles** | `references/principles/` | **211 + 20 부록** | **SOLID / GRASP / ISO 25010 / 12-Factor / Code Smells + 타입 시스템 / 동시성 이론 / Refactoring 기법 / SW 경제·추정 / 진화적 아키텍처 / 탄력성 / 문서화 / DORA / 성능 / Green Software + DB 기초 / SDLC 모델 / Scaled Agile / 직업 윤리 + Standards Mapping (SWEBOK/CS2023/DMBOK/OWASP/NIST-ISO) / Configuration Management (IEEE 828 SCMP) + HCI Methodology (ISO 9241 / Persona / Journey Map / Heuristic Eval) / Formal Methods (TLA+ / Alloy / Hoare / Model Checking)** | 23 파일 + index + 부록 [`micro-principles.md`](references/principles/micro-principles.md) (DRY/KISS/YAGNI/LoD/SoC/Tell-Don't-Ask/Composition/SSoT + Conway/Hyrum/Postel/Brooks/Hollywood/Boy Scout/Pareto/Goodhart/Cunningham/Inverse Conway, verify 카운트 외, `/principle <id>` 호출 가능). advisor 9 모드의 핵심 표준 인용 source |
...
합계 **1,247 항목 + 20 부록** (verify 카운트 기준).
...
**In-scope**: 디자인 패턴 (GoF 23 + 아키텍처/분산/신뢰성/동시성/통합/DDD 전술/데이터 접근/보안/테스트/Observability/AI-LLM/배포/캐싱/안티패턴/모바일/임베디드/게임/네트워크/크로스플랫폼/Offline-First/에러 처리/API 설계·스타일/Web 렌더링/상태 관리/FP/Reactive/레거시 코드/워크플로우/스트리밍/데이터 모델링/UI-UX/FinOps/테스트 전략/빌드·버전/MLOps/DX/블록체인/요구공학/MDM/DQ 거버넌스/Web Performance/Data Warehousing & BI/도메인 5종.
...
| 명령        | 패턴 (543)           | 알고리즘 (292)                  | 언어 (75)          | 보안 (106)             | 원칙 (211 + 20 부록)   | 품질 (20) |
|-------------|---------------------|--------------------------------|--------------------|-----------------------|-----------------------|-----------|
...
| `/dev-advisor swarm <module\|path>`        | "병렬 점검", "심층 분석", "swarm", "ultra audit", "7 에이전트 분석" | 7 Antigravity 서브에이전트 병렬 → reviewer 통합 + Top 10 |
| `/dev-advisor research <topic>`          | "논문", "최신 연구", "SOTA", "literature review", "학술 근거", "근거 논문" | 논문 매트릭스 + 카탈로그 cross-ref + 6 필드 (research 자산: [`references/research/`](references/research/)) |
| `/dev-advisor --help`                    | "도움말", "사용법"           | 모드/옵션/사용법 텍스트 |

자세한 절차·산출물·인용 references 는 **## 워크플로우** 섹션 참조. `full` 은 순차 통합 (7 모드 직렬 실행), `swarm` 은 병렬 통합 (7 Antigravity 서브에이전트 + reviewer)이다.
...
- **list**: JSON item 목록을 기준으로 도메인 카테고리 진입점 표 응답. 패턴/보안/원칙/품질은 `<domain>/index.md` 카테고리 매핑, 알고리즘은 `references/algorithms/index.md` 33 카테고리 표, 언어는 `references/languages/index.md` 우선순위 + 분야 매트릭스. **list <category>**: 카테고리 파일 `## 목차` 응답 (알고리즘만 카테고리 인자).
...
| 통합 점검 명시 키워드 (`전체 점검`, `종합 분석`, `모두 체크`, `6 도메인 분석`, `full audit`) | `full` 모드 | "이 모듈 전체 점검해줘" → `full <module>` |
...
| Antigravity 서브에이전트 사용 불가 환경 | `full` fallback (병렬 위임 불가 → 순차 직렬 실행) |
| 6 도메인 중 1~2 모드만 필요 | 개별 advisor 모드 단독 호출 권장 |

## 알고리즘 (292개, 33 카테고리)

33 카테고리 진입점 표 + 292 알고리즘 ID → 파일 매핑 + ID 명명 규칙 + `/algorithm <id>` 호출 동작 + 별칭 표 (`knn`, `bit`, `rb-tree`, `disjoint-set`, `a-star-search`, `lru-cache`, `breadth-first-search`, `depth-first-search` 등 8개) 는 [`references/algorithms/index.md`](references/algorithms/index.md) 참조.
...
**카테고리 진입점 + 543 패턴 ID → 파일 매핑**: [`references/patterns/index.md`](references/patterns/index.md) 참조.
...
Advisor 모드 (옵션) — 7 기본 + 2 통합:
  ...
  swarm            7 Antigravity 서브에이전트 병렬 → reviewer 통합 + Top 10

카탈로그 lookup (기존 보존):
  /pattern   <id|list|search>             543 패턴 (55 카테고리 — GoF + 아키텍처/분산/신뢰성/동시성/통합/DDD 전술·전략/데이터 접근/테스트/Observability/AI-LLM/배포/캐싱/안티패턴/모바일/임베디드/게임/네트워크/크로스플랫폼/Offline-First/에러 처리/API 설계·스타일/Web 렌더링/상태 관리/FP/Reactive/레거시 코드/워크플로우/스트리밍/데이터 모델링/UI-UX/FinOps/테스트 전략/빌드·버전/MLOps/DX/블록체인/요구공학/MDM/DQ 거버넌스/Web Performance/Data Warehousing & BI/Graphics Rendering/AR-VR-XR/Serverless-FaaS/HPC-Scientific/도메인 5종(Fintech·Healthcare·eCommerce·Logistics·IoT))
  /algorithm <id|list [category]|search>  292 알고리즘 (33 카테고리 — 정렬/탐색/그래프/DP/문자열/수학/자료구조/계산기하/플로우/매칭/암호/압축/게임AI/ML/확률/분산합의/분산알고/동시성/파싱 + DB 인덱스/DB 스토리지 엔진/DB 쿼리 옵티마이저/공간 인덱싱/검색·랭킹/부하 분산/OS 기초/이미지/신호/코덱)
  ...
  /principle <id|list|search>             211 + 20 부록 SW 공학 원칙
...
| `swarm`          | 모듈/path 필수 | 7 Antigravity 서브에이전트 병렬 결과 + reviewer 통합 보고 + Top 10 | Antigravity `invoke_subagent` 기반 병렬 실행, verifier 또는 code-reviewer/security-reviewer/architect hand-off |
...
- 변경 후 회귀 확인 → `verifier`
- 코드 스멜 deslop 정리 → `/oh-my-antigravity:ai-slop-cleaner` (`maintain` 모드 후속 스킬)
...
7 Antigravity 서브에이전트 병렬 위임 + reviewer 통합. 토큰 1만~2만 × 7 (병렬) + reviewer 5천 ≈ 8만~16만. 서브에이전트 사용이 불가한 환경에서는 `full` 로 fallback.

1. **입력 수집** — 모듈/path
2. **컨텍스트 분류** — 언어 자동 감지 + 규모 + 스택 + 도메인. 공통 컨텍스트를 7 서브에이전트에 공유 prompt 로 전달
3. **ULW 1차 발사** — Antigravity `invoke_subagent(Subagents=[{"TypeName": "executor", "Role": "Executor", "Prompt": "..."}])` 형태로 독립 분석 7명을 병렬 호출 (각 모드 1명: recommend / validate / security-audit / qa / qc / maintain / refactor).
...
6. **hand-off** — reviewer 판단에 따라 후속 에이전트 또는 스킬로 분기 (보안 위반 우세 → `security-reviewer`, 아키텍처 영향 → `architect`, PR 준비 → `code-reviewer`, 회귀 검증 → `verifier`, deslop → `/oh-my-antigravity:ai-slop-cleaner`)
...
### Parallel Results (7 Antigravity subagents)
...
advisor 9 모드의 트리거 조건 도달 시 Antigravity 전문 서브에이전트 또는 후속 스킬로 hand-off. 각 대상의 정식 입출력 계약·호출 형식은 [`references/handoff.md`](references/handoff.md) 참조.

| 대상 | 종류 | 트리거 | 후속 |
|---|---|---|---|
| `architect` | Antigravity subagent | `recommend` 결과 아키텍처 영향 ≥ 3 파일 / 계층 재설계 | `executor` 구현 |
| `code-reviewer` | Antigravity subagent | `refactor` 후 PR / `validate` P1 위반 수정 후 PR | `verifier` 회귀 |
| `security-reviewer` | Antigravity subagent | `security-audit` 결과 DREAD ≥ 8 / 컴플라이언스 carve-out / threat model 신규 | `verifier` + `code-reviewer` |
| `verifier` | Antigravity subagent | `refactor` / `maintain` / `security-audit` 적용 후, autopilot 종료 직전 | 통과 → 종료, 실패 → 재실행 |
| `/oh-my-antigravity:ai-slop-cleaner` | Antigravity skill | `maintain` Dispensables 그룹 비중 ≥ 40% | `code-reviewer` PR 리뷰 |

호출 형식은 `references/handoff.md` 참조.
...
- 패턴 (543) — [`references/patterns/index.md`](references/patterns/index.md)
- 알고리즘 (292) — [`references/algorithms/index.md`](references/algorithms/index.md)
...
- 원칙 (211 + 20 부록) — [`references/principles/index.md`](references/principles/index.md) · 미시 부록: [`micro-principles.md`](references/principles/micro-principles.md)
...
- [`references/handoff.md`](references/handoff.md) — Antigravity hand-off 상세 (architect/code-reviewer/security-reviewer/verifier + `/oh-my-antigravity:ai-slop-cleaner`) 정식 입출력 계약
