...
> **작성자**: zime + Claude (Opus 4.7)
> **검토자**: OMC critic (opus) + OMC architect (opus) + Codex rescue (GPT-5.4) + 사용자 피드백 (zime)
> **대상 스킬**: `~/.claude/skills/dev-advisor/`
...
| `references/handoff.md` | 수정 | `document-specialist` + `analyst` + `scientist` hand-off 계약 추가 (모두 `model="opus"` 강제) |
...
| **6. 검증/후속** | 3-source 교차 검증 로그(어느 소스에서 발견했는지) + 카탈로그 매핑 신뢰도(HIGH/MED/LOW) + OMC hand-off 후보 | 검증 로그 + 다음 액션 |

### 4.4.1 API → 도구 매핑 *(v0.3 — Codex P1: arXiv ToU 준수)*

| 소스 | 사용 도구 | 응답 포맷 | 파싱 책임 | 동시성 (Rate-limit aware) |
|------|----------|----------|----------|:------------------------:|
| arXiv | `WebFetch` | Atom 1.0 XML (JSON 아님) | Claude inline XML parsing | **직렬 throttled** (3초당 1요청, 단일 연결) *(v0.3 수정 — 병렬 → 직렬)* |
| Semantic Scholar | `WebFetch` + `fields=` 명시 | JSON | Claude inline JSON parsing | **직렬** (key 권장, shared throttling) |
| OpenAlex | `WebFetch` + `User-Agent: mailto:...` 헤더 (옵션) | JSON | Claude inline JSON parsing | 직렬 (🆓 완전 무료, mailto/key는 무료 옵션) *(v0.5)* |
...
- 도구 미사용 환경에서는 `document-specialist` 에이전트(`model="opus"`)로 fallback
...
| 필드 | research 모드 의미 |
|------|------------------|
| **선택/판정** | Top 3 핵심 논문 + 한 줄 종합 결론 (이 분야의 SOTA가 무엇인가) — **raw API 응답 기반, Claude cutoff 이전 지식 혼입 금지** |
...
- **인용수는 Semantic Scholar 또는 OpenAlex 응답에 있는 값만 사용** — Claude가 추정 금지
- **저자명은 API 응답 원문 그대로** — 표기 변형 금지
- **발표처(NeurIPS 등)는 API 응답의 `venue` 필드만 사용** — Claude가 추측 금지
...
- **Claude cutoff 이전 지식 혼입 금지** — "선택/판정" 6필드 작성 시 raw API 응답만 사용 *(v0.2 추가)*
- **재현 보고 출처 정책** *(v0.4 신규 — critic F12)*: 6 필드 "신뢰성·재현성 조건" 작성 시:
  ...
  - 3차: 응답 없음 → 명시적 `unknown (no reproduction report found)` 표기. Claude 추정 금지.
...
- 검색어 모호/0건 → `Task(subagent_type="oh-my-claudecode:document-specialist", model="opus", ...)` (외부 학술 문서 다중 소스 검색)
- 심층 비교 요청 → `Task(subagent_type="oh-my-claudecode:analyst", model="opus", ...)` 또는 `scientist`
...
| `OMC hand-off` 표 | `document-specialist` + `analyst` + `scientist` 행 추가 (모두 `model="opus"`) |
...
- `document-specialist` / `analyst` / `scientist` hand-off 정식 계약 신설 (모두 `model="opus"` 강제) *(v0.2 — F1 환각 에이전트 교체)*
...
## 9. OMC Hand-off 정책 *(v0.2 — F1 + F10 반영)*

| 트리거 | Hand-off 대상 | 모델 | 비고 |
|--------|-------------|------|------|
| 3-source 모두 0건 + 사용자가 재시도 요청 | `document-specialist` 에이전트 | **opus** | 외부 학술 문서 다중 소스 검색 (Google Scholar 등) |
| 한국어 논문 명시 요청 | `document-specialist` | **opus** | RISS/DBpia/KCI 외부 검색 |
| 카탈로그 매핑 신뢰도 HIGH ≥ 3 + 사용자가 구현 요청 | `executor` | **opus** | research 결과를 prompt 컨텍스트로 전달 |
| 심층 비교 분석 요청 ("이 두 논문 비교") | `analyst` 또는 `scientist` | **opus** | 분석 성격이므로 opus 강제 |
| 카탈로그 매핑 충돌 (한 논문이 여러 카탈로그 ID로 매핑) | `architect` | **opus** | 분류·관계 모델링 |
...
- Claude가 임의로 저자명·연도·인용수·발표처를 생성하는 것 절대 금지 (환각 방지 정책 §5.4 + §11.1)
...
| Claude 환각 (있지도 않은 논문 출력) | 중 | 매우 고 | §5.4 환각 방지 정책 + §11.1 사전 차단 정책 + DOI/ID 정규식 검증 + 3-source 교차 검증 로그 의무 |
...
- **메타 필드 직접 reference**: 저자/연도/인용수/발표처는 API 응답 raw dict의 직접 참조. Claude 추론·요약·번역 금지 (raw 그대로 출력)
- **URL 화이트리스트**: §10.2 도메인만 출력
- **카탈로그 매핑**: `catalog-index.json`의 실제 ID 목록과 대조 후에만 인용
- **6 필드 "선택/판정"**: API 응답 외 지식(Claude cutoff 이전) 혼입 금지
...
- [x] OMC critic + architect + Codex 검토
...
- [ ] **`analyst` / `scientist` 에이전트 존재 여부 확인** — `ls ~/.claude/agents/` + OMC marketplace listing
...
- [ ] **Phase 4-C**: OMC hand-off 표 (document-specialist + analyst + scientist)

### Phase 5 — 부가 자산 수정
- [ ] `references/output_templates.md` — research 모드 스켈레톤
- [ ] `references/examples.md` — 예시 K/L/M (research 호출 3 시나리오)
- [ ] `references/handoff.md` — `document-specialist` + `analyst` + `scientist` hand-off 계약 신설 (`model="opus"` 강제)
...
2차: Bash + curl (응답 신뢰성 사용자 검증 필수) — 🆓 무료
  ↓ 실패 (네트워크 차단 등)
3차: `document-specialist` 에이전트 위임 (model="opus") — 🆓 OMC 내장
```
...
- 원본 분석 세션: 2026-05-28 zime + Claude Opus 4.7
- v0.2 검토: OMC critic + architect (2026-05-28) + Codex rescue (보강 예정)
- 검토 후보 OMC 에이전트: `architect` (구조 설계) + `code-reviewer` (PR 리뷰) + `verifier` (Phase 6)
...
| HR-04 | 인용수 컬럼이 API 응답 raw value와 일치 (Claude 추정 X) | response trace | mock fixture 50쌍 |
...
| HR-07 | "선택/판정" 6필드에 cutoff 이전 지식 혼입 없음 (Claude pre-training 표지 검증) | LLM judge 또는 휴리스틱 | sample 20% |
