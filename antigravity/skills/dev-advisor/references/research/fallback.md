...
| 인용수 source 없음 | `n/a (no citation data)` — Claude 추정 절대 금지 |
...
**주의사항**:
- curl 응답은 raw JSON/XML이므로 Claude가 직접 파싱해야 한다.
...
1차·2차 모두 실패 시 (또는 3 API 모두 네트워크 차단 시) OMC 내장 에이전트에 위임한다.
...
- `model="opus"` 강제 (분석/검색 성격이므로 — CLAUDE.md 정책)
