...
- **LLM 런타임 자동 rate-limit 가정 금지**: Claude Code 런타임이 rate limit를 투명하게
...
|------|----------|----------|----------|:------:|
| **arXiv** | `WebFetch` | **Atom 1.0 XML** (JSON 아님 — 주의) | Claude inline XML parsing | **직렬 throttled** |
| **Semantic Scholar** | `WebFetch` + `fields=` 파라미터 명시 | JSON | Claude inline JSON parsing | **직렬** |
| **OpenAlex** | `WebFetch` + `User-Agent: mailto:...` 헤더 (옵션) | JSON | Claude inline JSON parsing | **직렬** (🆓 완전 무료) |
...
| `year` | 발행 연도 |
| `venue` | 발표처 (NeurIPS, ICML 등) — API 응답 raw 그대로 사용, Claude 추정 금지 |
...
Claude가 TLDR 내용을 추정하거나 생성하는 것을 절대 금지한다.
...
- 검색 결과를 파일로 저장하지 않는다
- `~/.claude/`, `.omc/`, 프로젝트 디렉토리 어디에도 결과를 쓰지 않는다
...
2차: Bash + curl 직접 호출
     ...
     └─ 실패 (네트워크 차단 등)
          ↓
3차: document-specialist 에이전트 위임 (model="opus")
