...
| 7 | **병렬 심층 점검 (Swarm)** | `swarm`          | 5 Claude Code 서브에이전트 병렬 결과 + reviewer 통합 + Top 10 (OMC ultrawork) |
...
5 카탈로그는 도메인 특성에 맞춘 비대칭 구조 (데이터 비대칭, 호출 표면 `list / search / <id>` 대칭) + advisor 7 모드 + `--help` (총 8 호출 표면 — `recommend / validate / refactor / maintain / security-audit / full / swarm / --help`). 자세한 동작은 **## 호출 인터페이스** 섹션.
...
| `/dev-advisor swarm <module\|path>`        | "병렬 점검", "심층 분석", "swarm", "ultra audit", "5 에이전트 분석" | 5 Claude Code 서브에이전트 병렬 → reviewer 통합 + Top 10 |
| `/dev-advisor --help`                    | "도움말", "사용법"           | 모드/옵션/사용법 텍스트 |

자세한 절차·산출물·인용 references 는 **## 워크플로우** 섹션 참조. `full` 은 순차 통합 (5 모드 직렬 실행), `swarm` 은 OMC ultrawork 기반 병렬 통합 (5 Claude Code 서브에이전트 + reviewer)이다.
...
**카테고리 진입점 + 508 패턴 ID → 파일 매핑**: [`references/patterns/index.md`](references/patterns/index.md) 참조.
...
Advisor 모드 (옵션) — 5 기본 + 2 통합:
  ...
  swarm            5 Claude Code 서브에이전트 병렬 → reviewer 통합 (OMC ultrawork) + Top 10
...
| `swarm`          | 모듈/path 필수 | 5 Claude Code 서브에이전트 병렬 결과 + reviewer 통합 보고 + Top 10 | OMC ultrawork + Claude Code `Task(subagent_type="oh-my-claudecode:<name>", model="opus", ...)` 기반 병렬 실행, verifier 또는 code-reviewer/security-reviewer/architect hand-off |
...
OMC ultrawork 기반 5 Claude Code 서브에이전트 병렬 위임 + reviewer 통합. 토큰 1만~2만 × 5 (병렬) + reviewer 5천 ≈ 6만~12만. ultrawork 또는 서브에이전트 사용이 불가한 환경에서는 `full` 로 fallback.

1. **입력 수집** — 모듈/path
2. **컨텍스트 분류** — 언어 자동 감지 + 규모 + 스택 + 도메인. 공통 컨텍스트를 5 서브에이전트에 공유 prompt 로 전달
3. **ULW 1차 발사** — Claude Code `Task(subagent_type="oh-my-claudecode:executor", model="opus", description="...", prompt="...")` 형태로 독립 분석 5명을 병렬 호출 (각 모드 1명: recommend / validate / security-audit / maintain / refactor). 분석·설계·리뷰 성격의 ULW 호출은 전역 `CLAUDE.md`의 OMC 모델 정책에 따라 `model="opus"` 를 명시한다.
...
### Parallel Results (5 Claude Code subagents)
...
advisor 7 모드의 트리거 조건 도달 시 Claude Code 전문 서브에이전트 또는 후속 OMC 스킬로 hand-off. 각 대상의 정식 입출력 계약·호출 형식은 [`references/handoff.md`](references/handoff.md) 참조.
