...
- **[필수]** `swarm` 또는 OMC hand-off에서 분석·설계·리뷰 에이전트 호출 시 `Task(subagent_type="oh-my-claudecode:<name>", model="opus", ...)` 형식을 사용한다.
...
`audit --mode=serial`은 단일 컨텍스트 7 모드 순차 통합, `audit --mode=parallel`은 OMC ultrawork 기반 7 서브에이전트 병렬 + reviewer 통합. `research`는 audit에 미포함 (독립). 라우팅 우선순위·audit `--mode` 자동 분기는 [`references/workflows/_routing.md`](references/workflows/_routing.md) 참조.
...
advisor 9 모드는 트리거 조건 도달 시 Claude Code 전문 서브에이전트 또는 별도 스킬로 hand-off 한다. 입출력 계약 상세는 [`references/handoff.md`](references/handoff.md) 참조.

| hand-off 대상 | 종류 | 트리거 요약 |
|---|---|---|
| `architect` | Task 서브에이전트 (`model="opus"`) | 아키텍처 영향 ≥ 3 파일 / 계층 재설계 / 도메인 분해 |
| `code-reviewer` | Task 서브에이전트 (`model="opus"`) | `refactor`/`validate` 후 PR / 표준·윤리 검토 |
| `security-reviewer` | Task 서브에이전트 (`model="opus"`) | DREAD ≥ 8 / 컴플라이언스 carve-out / threat model 등재 |
| `verifier` | Task 서브에이전트 (`model="opus"`) | 변경 적용 후 회귀 확인 / autopilot 종료 직전 |
| `designer` | Task 서브에이전트 (`model="opus"`) | UI 컴포넌트 영향 ≥ 3 화면 / HCI·UX 평가 |
| `planner` | Task 서브에이전트 (`model="opus"`) | 다단계 추정·로드맵 / SDLC·CM 계획 |
| `document-specialist` / `analyst` / `scientist` | Task 서브에이전트 (`model="opus"`) | research 모드 외부 문서·비교·통계 분석 |
| `/oh-my-claudecode:ai-slop-cleaner` | 스킬 호출 | `maintain` Dispensables 비중 ≥ 40% |
| **`data-advisor`** | **별도 스킬 호출** | **DMBOK 데이터 관리 주제(KA2/3/7/8/10) 감지 — 호출 `/data <id>` · `/mdm` · `/dwh` · `/dq` 또는 `Skill(skill="data-advisor")`** |

- **[필수]** 분석·설계·리뷰 성격 hand-off 는 `Task(subagent_type="oh-my-claudecode:<name>", model="opus", ...)` 형식을 사용한다 (bare ID 금지).
- **[필수]** `data-advisor` 는 Claude Code `Task` 서브에이전트가 아니라 **별도 스킬**이므로 `Task(...)` 가 아닌 스킬 호출(`/data ...` 또는 `Skill(skill="data-advisor")`)로 hand-off 한다.
...
- [`references/handoff.md`](references/handoff.md) — OMC hand-off 입출력 계약
