...
- **[필수]** Claude Code에서 이 스킬을 실행할 때는 `references/`와 `scripts/`를 현재 스킬 디렉터리 기준 상대 경로로 읽는다. 사용자 홈의 Codex 경로 또는 다른 런타임 경로를 하드코딩하지 않는다.
- **[필수]** `swarm` 또는 OMC hand-off에서 분석·설계·리뷰 에이전트를 호출할 때는 `Task(subagent_type="oh-my-claudecode:<name>", model="opus", ...)` 형식을 사용한다.
- **[필수]** `/language <id>` 응답은 `사용처/특징/장점/제약/실사용 예제/관련 문서`를 모두 포함하며, `관련 문서`에는 공식 문서 또는 표준·레퍼런스·툴링 문서 링크를 3개 이상 제공한다.
- **[권장]** 카탈로그 변경 후 `bash scripts/verify-references.sh`를 실행하고, 실패 항목이 1개 이상이면 수정 후 재실행한다.
...
| 9 | **병렬 심층 점검 (Swarm)** | `swarm`          | 7 Claude Code 서브에이전트 병렬 결과 + reviewer 통합 + Top 10 (OMC ultrawork) |
...
6 카탈로그는 도메인 특성에 맞춘 비대칭 구조 (데이터 비대칭, 호출 표면 `list / search / <id>` 대칭) + advisor 9 모드 + `--help` (총 10 호출 표면 — `recommend / validate / refactor / maintain / security-audit / qa / qc / full / swarm / --help`). 자세한 동작은 **## 호출 인터페이스** 섹션.

| 도메인 | 디렉토리 | 항목 수 | 용도 | 구조 |
|---|---|--:|---|---|
| Patterns   | `references/patterns/`   | 529 | 디자인·아키텍처 의사결정 (산업 도메인 포함, Web Performance / HPC-Scientific 신설) | 52 카테고리, `patterns/<category>.md` + `patterns/domains/<industry>.md` 평면 inline + 카테고리당 `## N.` 헤더 |
| Algorithms | `references/algorithms/` | 250 | 자료구조·연산·분산·동시성·파싱·OS·미디어 | 29 카테고리, `<a id>` anchor + 카테고리 목차 + `algorithms/index.md` 전역 ID 매핑 (4중 lookup) |
...
| **Principles** | `references/principles/` | **206 + 18 부록** | **SOLID / GRASP / ISO 25010 / 12-Factor / Code Smells + 타입 시스템 / 동시성 이론 / Refactoring 기법 / SW 경제·추정 / 진화적 아키텍처 / 탄력성 / 문서화 / DORA / 성능 / Green Software + SDLC 모델 / Scaled Agile / 직업 윤리 + Standards Mapping (SWEBOK/CS2023/DMBOK/OWASP/NIST-ISO) / Configuration Management (IEEE 828 SCMP) + HCI Methodology (ISO 9241 / Persona / Journey Map / Heuristic Eval) / Formal Methods (TLA+ / Alloy / Hoare / Model Checking)** | 22 파일 + index + 부록 [`micro-principles.md`](references/principles/micro-principles.md) (DRY/KISS/YAGNI/LoD/SoC/Tell-Don't-Ask/Composition/SSoT + Conway/Hyrum/Postel/Brooks/Hollywood/Boy Scout/Pareto/Goodhart/Cunningham/Inverse Conway, verify 카운트 외, `/principle <id>` 호출 가능). advisor 9 모드의 핵심 표준 인용 source |
...
합계 **1,186 항목 + 18 부록** (verify 카운트 기준).
...
**Out-of-scope**: 프레임워크별 가이드 (React/Spring/Django/Rails 등 product-specific) · 벤더 SDK API 사용법 (AWS/GCP/Azure/Firebase) · 클라우드 관리형 서비스 매핑 · 라이브러리 버전 마이그레이션 가이드 · DMBOK 데이터 관리 (MDM / DQ 거버넌스 / Data Warehousing & BI / DB 기초 / DB 인덱스·스토리지 엔진·쿼리 옵티마이저).

> **data-advisor 분리**: DMBOK 데이터 관리(MDM / DQ 거버넌스 / Data Warehousing & BI / DB 기초 / DB 인덱스·스토리지·옵티마이저)는 별도 `data-advisor` 스킬로 분리되었다. 해당 주제는 `/data <id>` 로 조회한다. dev-advisor 의 공간 인덱싱·검색·랭킹 알고리즘은 잔류한다.
...
| 명령        | 패턴 (529)           | 알고리즘 (250)                  | 언어 (75)          | 보안 (106)             | 원칙 (206 + 18 부록)   | 품질 (20) |
|-------------|---------------------|--------------------------------|--------------------|-----------------------|-----------------------|-----------|
...
| `/dev-advisor swarm <module\|path>`        | "병렬 점검", "심층 분석", "swarm", "ultra audit", "7 에이전트 분석" | 7 Claude Code 서브에이전트 병렬 → reviewer 통합 + Top 10 |
| `/dev-advisor --help`                    | "도움말", "사용법"           | 모드/옵션/사용법 텍스트 |

자세한 절차·산출물·인용 references 는 **## 워크플로우** 섹션 참조. `full` 은 순차 통합 (7 모드 직렬 실행), `swarm` 은 OMC ultrawork 기반 병렬 통합 (7 Claude Code 서브에이전트 + reviewer)이다.

### list / search / `<id>` 동작

- **JSON-only lookup**: ID/alias/file/anchor resolve는 `catalog-index.json`의 `items`, `aliases`, `lookup_keys`, `standards_mappings`만 사용한다. Markdown은 항목 본문과 인용의 source of truth로 읽되, lookup fallback으로 다시 파싱하지 않는다.
- **list**: JSON item 목록을 기준으로 도메인 카테고리 진입점 표 응답. 패턴/보안/원칙/품질은 `<domain>/index.md` 카테고리 매핑, 알고리즘은 `references/algorithms/index.md` 29 카테고리 표, 언어는 `references/languages/index.md` 우선순위 + 분야 매트릭스. **list <category>**: 카테고리 파일 `## 목차` 응답 (알고리즘만 카테고리 인자).
- **search**: JSON의 **ID·alias·영문명·한글명·태그·category** lookup key와 매칭하여 상위 ID 목록 반환. 매칭 우선순위: ID > alias > lookup key > 부분 문자열.
- **`<id>` 동작 (도메인별)**:
  - `/pattern <id>` → JSON에서 ID/alias 해석 → `references/patterns/<file>.md#<anchor>` 본문
  - `/algorithm <id>` → JSON에서 ID/alias 해석 → `references/algorithms/<file>.md#<anchor>` 본문
  - `/language <id>` → JSON에서 ID/alias 해석 → `references/languages/<lang>.md#<anchor>` 본문
  - `/security <id>` → JSON에서 ID/alias 해석 → `references/security/<file>.md#<anchor>` 본문
  - `/principle <id>` → JSON에서 `entry`/`category`/`appendix` 해석 → `<file>.md#<anchor>`. 별칭 예: `single-responsibility→srp`, `god-class→code-smell-large-class`, `12factor`/`twelve-factor→12-factor`, `dont-repeat-yourself→dry`. **미시 원칙 18 항목**은 `appendix`, `solid`/`grasp` 같은 도메인 진입점은 `category`로 같은 JSON 경로에서 lookup 가능.
  - `/quality <id>` → JSON에서 ID/alias 해석 → `references/quality/qa.md` 또는 `references/quality/qc.md#<anchor>` 본문. 별칭 예: `test-strategy→qa-test-strategy`, `quality-gate→qc-quality-gate`.
...
| OMC ultrawork 또는 Claude Code 서브에이전트 사용 불가 환경 | `full` fallback (병렬 위임 불가 → 순차 직렬 실행) |
| 6 도메인 중 1~2 모드만 필요 | 개별 advisor 모드 단독 호출 권장 |

## 알고리즘 (250개, 29 카테고리)

29 카테고리 진입점 표 + 250 알고리즘 ID → 파일 매핑 + ID 명명 규칙 + `/algorithm <id>` 호출 동작 + 별칭 표 (`knn`, `bit`, `rb-tree`, `disjoint-set`, `a-star-search`, `lru-cache`, `breadth-first-search`, `depth-first-search` 등 8개) 는 [`references/algorithms/index.md`](references/algorithms/index.md) 참조.
...
GoF 23 + 아키텍처/분산/신뢰성/동시성/통합/DDD 전술·전략/데이터 접근/테스트/Observability/AI-LLM/배포/캐싱/안티패턴/모바일/임베디드/게임/네트워크/크로스플랫폼/Offline-First/에러 처리/API 설계·스타일/Web 렌더링/상태 관리/FP/Reactive/레거시 코드/워크플로우/스트리밍/데이터 모델링/UI-UX/FinOps/테스트 전략/빌드·버전/MLOps/DX/블록체인/요구공학/Web Performance/도메인 5종.

**카테고리 진입점 + 529 패턴 ID → 파일 매핑**: [`references/patterns/index.md`](references/patterns/index.md) 참조.
...
Advisor 모드 (옵션) — 7 기본 + 2 통합:
  ...
  swarm            7 Claude Code 서브에이전트 병렬 → reviewer 통합 (OMC ultrawork) + Top 10

카탈로그 lookup (기존 보존):
  /pattern   <id|list|search>             529 패턴 (52 카테고리 — GoF + 아키텍처/분산/신뢰성/동시성/통합/DDD 전술·전략/데이터 접근/테스트/Observability/AI-LLM/배포/캐싱/안티패턴/모바일/임베디드/게임/네트워크/크로스플랫폼/Offline-First/에러 처리/API 설계·스타일/Web 렌더링/상태 관리/FP/Reactive/레거시 코드/워크플로우/스트리밍/데이터 모델링/UI-UX/FinOps/테스트 전략/빌드·버전/MLOps/DX/블록체인/요구공학/Web Performance/Graphics Rendering/AR-VR-XR/Serverless-FaaS/HPC-Scientific/도메인 5종(Fintech·Healthcare·eCommerce·Logistics·IoT))
  /algorithm <id|list [category]|search>  250 알고리즘 (29 카테고리 — 정렬/탐색/그래프/DP/문자열/수학/자료구조/계산기하/플로우/매칭/암호/압축/게임AI/ML/확률/분산합의/분산알고/동시성/파싱 + 공간 인덱싱/검색·랭킹/부하 분산/OS 기초/이미지/신호/코덱)
  ...
  /principle <id|list|search>             206 + 18 부록 SW 공학 원칙
                                          ...
                                          P0 신설 3: SDLC Models 7 / Scaled Agile 6 / Professional Ethics 6 (Database Fundamentals 8 은 data-advisor 로 이관)
...
| `swarm`          | 모듈/path 필수 | 7 Claude Code 서브에이전트 병렬 결과 + reviewer 통합 보고 + Top 10 | OMC ultrawork + Claude Code `Task(subagent_type="oh-my-claudecode:<name>", model="opus", ...)` 기반 병렬 실행, verifier 또는 code-reviewer/security-reviewer/architect hand-off |
...
6. **검증/후속 계획** — 회귀 테스트 + 재검증 명령 + OMC 에이전트 hand-off 후보
...
- 변경 후 회귀 확인 → `verifier`
- 코드 스멜 deslop 정리 → `/oh-my-claudecode:ai-slop-cleaner` (`maintain` 모드 후속 스킬)
...
OMC ultrawork 기반 7 Claude Code 서브에이전트 병렬 위임 + reviewer 통합. 토큰 1만~2만 × 7 (병렬) + reviewer 5천 ≈ 8만~16만. ultrawork 또는 서브에이전트 사용이 불가한 환경에서는 `full` 로 fallback.
...
advisor 9 모드의 트리거 조건 도달 시 Claude Code 전문 서브에이전트 또는 후속 OMC 스킬로 hand-off. 각 대상의 정식 입출력 계약·호출 형식은 [`references/handoff.md`](references/handoff.md) 참조.

| 대상 | 종류 | 트리거 | 후속 |
|---|---|---|---|
| `architect` | Claude Code subagent | `recommend` 결과 아키텍처 영향 ≥ 3 파일 / 계층 재설계 | `executor` 구현 |
| `code-reviewer` | Claude Code subagent | `refactor` 후 PR / `validate` P1 위반 수정 후 PR | `verifier` 회귀 |
| `security-reviewer` | Claude Code subagent | `security-audit` 결과 DREAD ≥ 8 / 컴플라이언스 carve-out / threat model 신규 | `verifier` + `code-reviewer` |
| `verifier` | Claude Code subagent | `refactor` / `maintain` / `security-audit` 적용 후, autopilot 종료 직전 | 통과 → 종료, 실패 → 재실행 |
| `/oh-my-claudecode:ai-slop-cleaner` | OMC skill | `maintain` Dispensables 그룹 비중 ≥ 40% | `code-reviewer` PR 리뷰 |

호출 형식 + 모델 정책은 전역 `CLAUDE.md` OMC/ULW 분석·설계 모델 정책 + `references/handoff.md` 참조.
...
- 패턴 (529) — [`references/patterns/index.md`](references/patterns/index.md)
- 알고리즘 (250) — [`references/algorithms/index.md`](references/algorithms/index.md)
...
- 원칙 (206 + 18 부록) — [`references/principles/index.md`](references/principles/index.md) · 미시 부록: [`micro-principles.md`](references/principles/micro-principles.md)
...
- [`references/handoff.md`](references/handoff.md) — OMC hand-off 상세 (architect/code-reviewer/security-reviewer/verifier + `/oh-my-claudecode:ai-slop-cleaner`) 정식 입출력 계약
...
`catalog-index.json`은 Markdown reference 에서 생성되는 JSON-only lookup 색인이다. 카탈로그 항목, alias, 표준 매핑을 바꾼 뒤에는 `python3 scripts/generate-catalog-index.py`로 JSON을 갱신하고, `python3 scripts/generate-catalog-index.py --check`로 stale 여부를 확인한다. 실제 lookup 경로는 `python3 scripts/lookup-catalog.py principle srp`, `python3 scripts/lookup-catalog.py standard API3`처럼 JSON만 읽는 helper로 검증한다. JSON은 수동 편집하지 않는다.
