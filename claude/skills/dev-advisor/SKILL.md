...
- **JSON 우선**: `catalog-index.json`의 `items`, `aliases`, `lookup_keys`로 먼저 해석한다. Markdown index는 JSON에 없는 예외 상황의 fallback 및 본문 인용용 source of truth로만 읽는다.
...
`catalog-index.json`은 Markdown reference 에서 생성되는 lookup 색인이다. 카탈로그 항목을 바꾼 뒤에는 `python3 scripts/generate-catalog-index.py`로 JSON을 갱신하고, `python3 scripts/generate-catalog-index.py --check`로 stale 여부를 확인한다. 실제 lookup 경로는 `python3 scripts/lookup-catalog.py principle srp`처럼 JSON만 읽는 helper로 검증한다. JSON은 수동 편집하지 않는다.
