...
dev-advisor advisor 7 모드 → 트리거 조건 도달 → Claude Code 전문 서브에이전트 또는 후속 OMC 스킬 hand-off. 각 대상의 정식 입출력 계약. SKILL.md `## OMC hand-off` 요약 표에서 참조.

---

## OMC hand-off

advisor 7 모드는 다음 트리거 조건 도달 시 Claude Code 전문 서브에이전트 또는 후속 OMC 스킬로 hand-off 한다. `architect` / `code-reviewer` / `security-reviewer` / `verifier` 는 Claude Code `Task` 대상이고, `/oh-my-claudecode:ai-slop-cleaner` 는 별도 스킬 호출 대상이다.
...
아래 형식은 Claude Code 서브에이전트 역할(`architect`, `code-reviewer`, `security-reviewer`, `verifier`)에만 적용한다. `/oh-my-claudecode:ai-slop-cleaner` 는 스킬 호출로 실행한다.
