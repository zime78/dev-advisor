...
advisor 7 모드는 다음 트리거 조건 도달 시 Claude Code 전문 서브에이전트 또는 후속 OMC 스킬로 hand-off 한다. `architect` / `code-reviewer` / `security-reviewer` / `verifier` / `designer` / `planner` 는 Claude Code `Task` 대상이고, `/oh-my-claudecode:ai-slop-cleaner` 는 별도 스킬 호출 대상이다.
