...
## 디자인 패턴 (15 카테고리 + Phase 2 확장 32 카테고리 + P0 신설 2 카테고리 + P1 신설 2 카테고리 + P2 신설 3 카테고리 + P3 신설 1 카테고리)

GoF 23 + 아키텍처/분산/신뢰성/동시성/통합 + DDD 전술/데이터 접근/테스트/Observability/AI-LLM/배포/캐싱 (기존 159) + Phase 2 확장 (안티패턴·플랫폼·산업 도메인 등 32 카테고리) + P0 신설 (MDM·DQ 거버넌스 2 카테고리) + P1 신설 (Web Performance·Data Warehousing & BI 2 카테고리) + P2 신설 (Graphics Rendering·AR/VR/XR·Serverless/FaaS 3 카테고리) + P3 신설 (HPC/과학 계산 1 카테고리, 총 55 카테고리 / 547 패턴).

> **보안 패턴은 별도 도메인으로 분리되었습니다**: [`../security/`](../security/) (15 파일, 메타 + 12 sub + Privacy Engineering + 컴플라이언스). dev-catalog는 6중 카탈로그(patterns / algorithms / languages / security / principles / quality)로 격상되었다.
...
### Governance (거버넌스 — 9 카테고리)

코드 품질·에러 모델·요구공학·레거시 변환·비용·DX·데이터 거버넌스 등 조직/프로세스 차원의 의사결정 패턴. **P0 신설**: MDM / DQ 거버넌스 2 카테고리.
...
| [Master Data Management (P0)](master-data-management.md) | master-data-management.md | Golden Record, Survivorship Rules, Match-Merge, Data Steward, Reference Data, Hierarchy Management |
| [Data Quality & Governance (P0)](data-quality-governance.md) | data-quality-governance.md | DQ 6 Dimensions, Data Lineage, Data Catalog, Stewardship RACI, OpenLineage Standard, DQ Validation Tools |
...
### Performance & BI (성능·데이터 분석 — 2 카테고리, P1 신설)

웹 성능 측정·렌더링 최적화·번들 분석 + 데이터 웨어하우스·차원 모델링·BI 분석.

| 카테고리 | 파일 | 대표 항목 |
|---------|------|-----------|
| [Web Performance (P1)](web-performance.md) | web-performance.md | Core Web Vitals (LCP/FID/INP/CLS/TBT), Critical Rendering Path, Resource Hints, RUM Monitoring, Lighthouse/WebPageTest, Bundle Analysis |
| [Data Warehousing & BI (P1)](data-warehousing-bi.md) | data-warehousing-bi.md | Kimball Star/Snowflake, SCD Types 1/2/3/4/6, Fact Table 4종, OLAP Cube vs Tabular, Lakehouse (Iceberg/Delta/Hudi), dbt Modeling |
| [Graphics / Rendering (P2)](graphics-rendering.md) | graphics-rendering.md | Rasterization Pipeline, Ray Tracing (BVH/Path Tracing), Z-Buffering (Depth Test), PBR (Cook-Torrance), Forward vs Deferred Rendering |
| [AR / VR / XR (P2)](ar-vr-xr.md) | ar-vr-xr.md | 6DoF Tracking, SLAM, Foveated Rendering, Reprojection (ATW/ASW), XR 플랫폼 (OpenXR/ARKit/ARCore/visionOS) |
| [Serverless / FaaS (P2)](serverless-faas.md) | serverless-faas.md | Cold Start Mitigation, Provisioned Concurrency, Event Source Mapping, Step Functions Durable Workflow, Cold vs Hot Path |
| [HPC / Scientific Computing (P3)](hpc-scientific.md) | hpc-scientific.md | MPI, OpenMP, CUDA/HIP, SLURM, BLAS/LAPACK/ScaLAPACK, NUMA / Roofline Model |

또한 P2 본문 확장으로 다음 기존 카테고리에 신규 매트릭스 섹션이 추가됨:
- [architectural.md](architectural.md) → `## 17. Micro-Frontend (마이크로 프론트엔드)`
- [integration.md](integration.md) → `## 17. Message Broker Selection Matrix (메시지 브로커 선택 매트릭스)`
- [distributed.md](distributed.md) → `## 10. TCC (Try-Confirm-Cancel)` + `## 11. Distributed Transaction Selection Matrix (Saga/2PC/3PC/TCC/Outbox 매트릭스)`

P3 본문 확장으로 다음 기존 카테고리에 신규 매트릭스 섹션이 추가됨:
- [mobile-app.md](mobile-app.md) → `## 14. App Store Compliance (앱 스토어 심사 준수)` + `## 15. Mobile Advertising & Attribution (모바일 광고·어트리뷰션)`
...
| context-map-strategic | Context Map | 컨텍스트 맵 |
...
### Master Data Management (master-data-management.md, P0) — 6
| ID | 영문명 | 한글명 |
|----|--------|--------|
...
### Data Quality & Governance (data-quality-governance.md, P0) — 6
| ID | 영문명 | 한글명 |
|----|--------|--------|
...
### Web Performance (web-performance.md, P1) — 6
| ID | 영문명 | 한글명 |
|----|--------|--------|
| core-web-vitals | Core Web Vitals (LCP / FID / INP / CLS / TBT) | 코어 웹 바이탈 |
| critical-rendering-path | Critical Rendering Path | 핵심 렌더링 경로 |
| resource-hints | Resource Hints & Priority Hints | 리소스 힌트 |
| rum-monitoring | RUM (Real User Monitoring) | 실사용자 모니터링 |
| lighthouse-webpagetest | Lighthouse / WebPageTest / PageSpeed Insights | 성능 측정 도구 |
| bundle-analysis | Bundle Analysis (Tree shaking / Code splitting / Module Federation) | 번들 분석 |

### Data Warehousing & BI (data-warehousing-bi.md, P1) — 6
| ID | 영문명 | 한글명 |
|----|--------|--------|
| kimball-star-snowflake | Kimball Star / Snowflake Schema | 차원 모델링 (Kimball) |
| scd-types | Slowly Changing Dimensions (Type 1/2/3/4/6 + Bridge Table) | SCD 타입 |
| fact-table-types | Fact Table 4종 (Transaction / Periodic / Accumulating / Factless) | 팩트 테이블 |
| olap-cube-tabular | OLAP Cube vs Tabular (MOLAP / ROLAP / HOLAP / Vertipaq) | OLAP 큐브/테이블 |
| lakehouse-iceberg-delta-hudi | Lakehouse (Apache Iceberg / Delta Lake / Apache Hudi) | 레이크하우스 |
| dbt-modeling | dbt — Model / Snapshot / Test / Macro / Seed / Layer | dbt 모델링 |

### Graphics / Rendering (graphics-rendering.md, P2) — 5
| ID | 영문명 | 한글명 |
|----|--------|--------|
| rasterization-pipeline | Rasterization Pipeline | 래스터화 파이프라인 |
| ray-tracing-bvh-pathtracing | Ray Tracing / BVH / Path Tracing | 레이 트레이싱 / BVH / 경로 추적 |
| z-buffering-depth-test | Z-Buffering / Depth Test | Z-버퍼링 / 깊이 테스트 |
| pbr-cook-torrance | PBR (Cook-Torrance BRDF) | 물리 기반 렌더링 |
| forward-vs-deferred-rendering | Forward vs Deferred Rendering | 포워드 vs 디퍼드 렌더링 |

### AR / VR / XR (ar-vr-xr.md, P2) — 5
| ID | 영문명 | 한글명 |
|----|--------|--------|
| xr-6dof-tracking | XR 6DoF Tracking (IMU + Optical Fusion) | 6자유도 추적 |
| xr-slam | SLAM (Simultaneous Localization and Mapping) | 동시적 위치추정·지도작성 |
| foveated-rendering | Foveated Rendering (Fixed / Eye-Tracked / DLSS) | 포비티드 렌더링 |
| xr-reprojection-atw-asw | Reprojection (ATW / ASW / Spacewarp) | 재투영 |
| xr-platforms-openxr-arkit-arcore-visionos | XR Platforms (OpenXR / ARKit / ARCore / visionOS) | XR 플랫폼 |

### Serverless / FaaS (serverless-faas.md, P2) — 5
| ID | 영문명 | 한글명 |
|----|--------|--------|
| faas-cold-start-mitigation | FaaS Cold Start Mitigation | 콜드 스타트 완화 |
| faas-provisioned-concurrency | Provisioned Concurrency / Reserved Capacity | 사전 프로비저닝 동시성 |
| faas-event-source-mapping | Event Source Mapping (SQS / Kinesis / DynamoDB Streams) | 이벤트 소스 매핑 |
| step-functions-durable-workflow | Step Functions / Durable Workflow / Temporal | 지속성 워크플로우 |
| faas-cold-vs-hot-path | Cold vs Hot Path (트래픽 패턴 기반 선택) | 콜드/핫 경로 선택 |

### P2 본문 확장 (기존 카테고리에 추가)
| 카테고리 | 확장 anchor |
|---------|------------|
| architectural.md | `micro-frontend` (## 17) |
| integration.md | `message-broker-selection-matrix` (## 17) |
| distributed.md | `tcc-try-confirm-cancel` (## 10), `distributed-transaction-selection-matrix` (## 11) |

### HPC / Scientific Computing (hpc-scientific.md, P3) — 6
| ID | 영문명 | 한글명 |
|----|--------|--------|
| hpc-mpi | MPI (Message Passing Interface) | 분산 메시지 전달 |
| openmp-parallel | OpenMP — 공유 메모리 병렬 | 공유 메모리 병렬 |
| cuda-hip-hpc | CUDA / HIP — GPU 가속 | GPU 가속 |
| slurm-workload-manager | SLURM Workload Manager | HPC 작업 스케줄러 |
| blas-lapack-scalapack | BLAS / LAPACK / ScaLAPACK | 선형대수 라이브러리 |
| numa-roofline-model | NUMA / Roofline Model | NUMA·Roofline 성능 모델 |

### P3 본문 확장 (기존 카테고리에 추가)
| 카테고리 | 확장 anchor |
|---------|------------|
| mobile-app.md | `app-store-compliance` (## 14), `mobile-advertising-attribution` (## 15) |
...
- 마스터 데이터 통합·중복 제거·골든 레코드: [Master Data Management (P0)](master-data-management.md) (Golden Record/Survivorship/Match-Merge/Data Steward/Reference Data/Hierarchy)
- 데이터 품질·리니지·카탈로그·스튜어드십: [Data Quality & Governance (P0)](data-quality-governance.md) (DQ 6 Dimensions/Data Lineage/Data Catalog/Stewardship RACI/OpenLineage/Validation Tools)
...
**Performance & BI (성능·데이터 분석, P1 신설)**
- 웹 성능 측정·렌더링 최적화·번들 분석: [Web Performance](web-performance.md) (Core Web Vitals/CRP/Resource Hints/RUM/Lighthouse/Bundle Analysis)
- 데이터 웨어하우스·차원 모델링·BI 분석: [Data Warehousing & BI](data-warehousing-bi.md) (Kimball Star/SCD Types/Fact Table 4종/OLAP/Lakehouse/dbt)

**Graphics / XR / Serverless (P2 신설)**
- 3D 렌더링·셰이딩·셰이더 파이프라인: [Graphics / Rendering](graphics-rendering.md) (Rasterization/Ray Tracing/Z-Buffer/PBR/Forward vs Deferred)
- AR/VR/XR 시스템·6DoF·SLAM·포비티드 렌더링: [AR/VR/XR](ar-vr-xr.md) (6DoF/SLAM/Foveated/ATW-ASW/OpenXR/ARKit/ARCore/visionOS)
- 서버리스·FaaS·콜드 스타트·이벤트 소스: [Serverless / FaaS](serverless-faas.md) (Cold Start/Provisioned Concurrency/Event Source Mapping/Step Functions/Cold vs Hot)
- 마이크로 프론트엔드: [architectural.md#micro-frontend](architectural.md#micro-frontend)
- 메시지 브로커 선택: [integration.md#message-broker-selection-matrix](integration.md#message-broker-selection-matrix)
- 분산 트랜잭션 선택 매트릭스: [distributed.md#distributed-transaction-selection-matrix](distributed.md#distributed-transaction-selection-matrix), TCC: [distributed.md#tcc-try-confirm-cancel](distributed.md#tcc-try-confirm-cancel)

**HPC / Scientific Computing (P3 신설)**
- 분산 메모리 메시지 전달·SPMD 병렬: [HPC / Scientific](hpc-scientific.md) (MPI/OpenMP/CUDA-HIP/SLURM/BLAS-LAPACK-ScaLAPACK/NUMA-Roofline)
- 앱 스토어 심사 준수: [mobile-app.md#app-store-compliance](mobile-app.md#app-store-compliance)
- 모바일 광고·어트리뷰션 (IDFA/AAID/ATT/Data Safety): [mobile-app.md#mobile-advertising-attribution](mobile-app.md#mobile-advertising-attribution)

### 별칭 표 (Phase 2 확장 — P1 신설 부분)

| 별칭 | Primary ID | 비고 |
|------|-----------|------|
| cwv | core-web-vitals | P1 Web Performance (Core Web Vitals 약어) |
| lcp | core-web-vitals | P1 Web Performance (LCP) |
| fid | core-web-vitals | P1 Web Performance (FID) |
| inp | core-web-vitals | P1 Web Performance (INP) |
| cls | core-web-vitals | P1 Web Performance (CLS) |
| crp | critical-rendering-path | P1 Web Performance (CRP 약어) |
| preload-prefetch | resource-hints | P1 Web Performance (preload/prefetch) |
| rum | rum-monitoring | P1 Web Performance (Real User Monitoring) |
| lighthouse | lighthouse-webpagetest | P1 Web Performance (Lighthouse) |
| webpagetest | lighthouse-webpagetest | P1 Web Performance (WebPageTest) |
| tree-shaking | bundle-analysis | P1 Web Performance (Tree shaking) |
| code-splitting | bundle-analysis | P1 Web Performance (Code splitting) |
| dwh | kimball-star-snowflake | P1 Data Warehousing (DWH 약어) |
| star-schema | kimball-star-snowflake | P1 Data Warehousing (Star Schema) |
| snowflake-schema | kimball-star-snowflake | P1 Data Warehousing (Snowflake Schema) |
| kimball | kimball-star-snowflake | P1 Data Warehousing (Kimball) |
| scd | scd-types | P1 Data Warehousing (SCD 약어) |
| slowly-changing-dimensions | scd-types | P1 Data Warehousing (정식) |
| fact-table | fact-table-types | P1 Data Warehousing (Fact Table) |
| olap | olap-cube-tabular | P1 Data Warehousing (OLAP 약어) |
| molap | olap-cube-tabular | P1 Data Warehousing (MOLAP) |
| rolap | olap-cube-tabular | P1 Data Warehousing (ROLAP) |
| tabular | olap-cube-tabular | P1 Data Warehousing (Tabular / Vertipaq) |
| lakehouse | lakehouse-iceberg-delta-hudi | P1 Data Warehousing (Lakehouse 단축) |
| iceberg | lakehouse-iceberg-delta-hudi | P1 Data Warehousing (Apache Iceberg) |
| delta-lake | lakehouse-iceberg-delta-hudi | P1 Data Warehousing (Delta Lake) |
| hudi | lakehouse-iceberg-delta-hudi | P1 Data Warehousing (Apache Hudi) |
| dbt | dbt-modeling | P1 Data Warehousing (dbt) |
| data-build-tool | dbt-modeling | P1 Data Warehousing (dbt 정식) |
| mpi | hpc-mpi | P3 HPC (MPI 단축) |
| message-passing-interface | hpc-mpi | P3 HPC (MPI 정식) |
| cuda-hpc | cuda-hip-hpc | P3 HPC (CUDA 단축) |
| hip-hpc | cuda-hip-hpc | P3 HPC (AMD HIP 단축) |
| openmp | openmp-parallel | P3 HPC (OpenMP 단축) |
| slurm | slurm-workload-manager | P3 HPC (SLURM 단축) |
| blas | blas-lapack-scalapack | P3 HPC (BLAS 단축) |
| lapack | blas-lapack-scalapack | P3 HPC (LAPACK 단축) |
| scalapack | blas-lapack-scalapack | P3 HPC (ScaLAPACK 단축) |
| numa | numa-roofline-model | P3 HPC (NUMA 단축) |
| roofline | numa-roofline-model | P3 HPC (Roofline 단축) |
| app-store | app-store-compliance | P3 mobile-app ## 14 (앱 스토어 단축) |
| play-console | app-store-compliance | P3 mobile-app ## 14 (Google Play) |
| idfa | mobile-advertising-attribution | P3 mobile-app ## 15 (IDFA) |
| att | mobile-advertising-attribution | P3 mobile-app ## 15 (App Tracking Transparency) |
| skadnetwork | mobile-advertising-attribution | P3 mobile-app ## 15 (SKAdNetwork) |
