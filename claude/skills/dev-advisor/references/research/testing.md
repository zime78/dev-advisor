...
**위협 2 — 환각**: Claude가 API 응답 외의 지식(pre-training cutoff 이전 지식, 추정값, 존재하지 않는 식별자)을 매트릭스 행에 혼입하는 경우.
...
PLAN v0.3에서 Codex 권고에 따라 0.85 → 0.90으로 상향되었다.  
...
for f in /Users/zime/.claude/skills/dev-advisor/references/research/fixtures/*/*.json; do
...
grep -q "research" ~/.claude/skills/dev-advisor/SKILL.md \
...
TRIGGER_COUNT=$(grep -cE '논문|research|SOTA|literature review|학술' \
  ~/.claude/skills/dev-advisor/SKILL.md || true)
[ "$TRIGGER_COUNT" -ge 2 ] \
...
grep -q "research" ~/.claude/skills/dev-advisor/references/output_templates.md \
...
EXAMPLE_COUNT=$(grep -cE '예시 K|예시 L|예시 M|Example K|Example L|Example M' \
  ~/.claude/skills/dev-advisor/references/examples.md || true)
[ "$EXAMPLE_COUNT" -ge 3 ] \
...
for AGENT in "document-specialist" "analyst" "scientist"; do
  grep -q "$AGENT" ~/.claude/skills/dev-advisor/references/handoff.md \
...
FIXTURE_ROOT=~/.claude/skills/dev-advisor/references/research/fixtures
...
SOURCES_MD=~/.claude/skills/dev-advisor/references/research/sources.md
...
RESEARCH_DIR=~/.claude/skills/dev-advisor/references/research
...
- **환각 정의**: API 응답 외의 값(Claude pre-training 지식, 추정값, 존재하지 않는 식별자)이 매트릭스 행에 혼입된 경우
...
| HR-04 | 인용수 컬럼이 API 응답 raw value와 일치 (Claude 추정값 X) | response trace | mock fixture 50쌍 전체 | FAIL — CI 차단 대상 |
...
인용수는 API 응답 raw value를 그대로 출력해야 한다. Claude가 추정하거나 반올림·가공한 값은 환각으로 판정한다.
...
카탈로그 매핑은 Claude가 임의로 ID를 생성하는 것을 방지하기 위해 `catalog-index.json`의 실제 ID 목록과 대조한다.

```bash
# HR-08 검증 (bash)
CATALOG=~/.claude/skills/dev-advisor/catalog-index.json
...
bash /Users/zime/.claude/skills/dev-advisor/scripts/verify-references.sh --check research

# 3. 통합 테스트 — fixtures 기반 mock dry-run + 골든 비교
echo ""
echo "=== Integration Tests (fixtures) ==="
FIXTURE_ROOT=/Users/zime/.claude/skills/dev-advisor/references/research/fixtures
...
python3 /Users/zime/.claude/skills/dev-advisor/scripts/generate-catalog-index.py --check

# 5. 환각 회귀 — HR-01~HR-10
echo ""
echo "=== Hallucination Regression (HR-01~HR-10) ==="
bash /Users/zime/.claude/skills/dev-advisor/scripts/verify-references.sh --check research-hallucination
```
