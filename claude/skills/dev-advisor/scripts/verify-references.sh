...
#   [13] semantic index / standards mapping 검증
#
# 빠른 선택 검증:
#   --check quality  QA/QC 카탈로그와 문서 노출만 검증
#   --check today    오늘 확장 P0/P1/P2/P3 항목과 QA/QC만 검증
#   --check semantic index ↔ anchor ↔ standards mapping 의미 검증
set -euo pipefail
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --check)
      if [ "$#" -lt 2 ]; then
        echo "FATAL: --check requires one of: all, today, quality, semantic" >&2
        exit 1
      fi
      CHECK_SCOPE="$2"
      shift 2
      ;;
    --help|-h)
      echo "Usage: $0 [--check all|today|quality|semantic]"
      exit 0
      ;;
    *)
      echo "FATAL: unknown argument: $1" >&2
      echo "Usage: $0 [--check all|today|quality|semantic]" >&2
      exit 1
      ;;
  esac
done
...
case "${CHECK_SCOPE}" in
  all|today|quality|semantic) ;;
  *)
    echo "FATAL: invalid --check value: ${CHECK_SCOPE} (expected all, today, quality, semantic)" >&2
    exit 1
    ;;
esac
...
def mapping_row_matches(path, item_id, name, ordinal):
    if not path.exists():
        return False
    ...
    for candidate in candidates:
        ...
        if candidate in anchors:
            return True
        if any(anchor.startswith(candidate + "-") or anchor.endswith("-" + candidate) for anchor in anchors):
            return True
...
def parse_domain_mapping(domain, start_re, stop_re):
    ...
    for line in index_path.read_text(encoding="utf-8").splitlines():
        ...
        if not row:
            continue
        ordinal += 1
        rows.append((strip_md(row.group(1)), refs / domain / current_file, strip_md(row.group(2)), ordinal))
    return rows

def check_domain_mapping(domain, start_re, stop_re):
    rows = parse_domain_mapping(domain, start_re, stop_re)
    seen = {}
    for item_id, path, name, ordinal in rows:
        ...
        if not mapping_row_matches(path, item_id, name, ordinal):
            issues.append(f"{domain}/index.md: `{item_id}` -> {path.relative_to(refs)} anchor/heading 불일치")
    for item_id, locations in sorted(seen.items()):
        if len(locations) > 1:
            issues.append(f"{domain}/index.md: 중복 ID `{item_id}` ({', '.join(locations)})")
    ...
    if actual_anchor_count > len(indexed):
        warnings.append(f"{domain}: 실제 anchor/heading 후보 {actual_anchor_count}개, index ID {len(indexed)}개 (미등재 anchor는 warning)")
    return len(rows)

patterns_rows = check_domain_mapping("patterns", r"^## 패턴 ID", r"^## 패턴 선택")
security_rows = check_domain_mapping("security", r"^## 보안 ID", r"^## 표준 인용")

def check_quality_mapping():
    rows = []
    index_path = refs / "quality" / "index.md"
    for line in index_path.read_text(encoding="utf-8").splitlines():
        row = re.match(r"^\|\s*`([^`]+)`\s*\|\s*\[[^\]]+\]\(([^)#]+\.md)#([^)]*)\)", line)
        if row:
    ...
    for item_id, file_name, fragment in rows:
        ...
        if item_id != fragment:
            issues.append(f"quality/index.md: `{item_id}` 링크 fragment `{fragment}` 불일치")
        if not path.exists() or not anchor_matches(fragment, anchors_for(path)[0]):
            issues.append(f"quality/index.md: `{item_id}` -> {file_name}#{fragment} anchor 누락")
    for item_id, locations in sorted(seen.items()):
        if len(locations) > 1:
            issues.append(f"quality/index.md: 중복 ID `{item_id}` ({', '.join(locations)})")
    return len(rows)

quality_rows = check_quality_mapping()

def check_principles_aliases():
    ...
    for alias, primaries in sorted(aliases.items()):
        if len(primaries) > 1:
            issues.append(f"principles/index.md: 중복 별칭 `{alias}` ({', '.join(primaries)})")
    return len(aliases)

principles_alias_rows = check_principles_aliases()

def check_standards_mapping():
    path = refs / "principles" / "standards-mapping.md"
    text = path.read_text(encoding="utf-8")
    link_re = re.compile(r"`((?:security|patterns|principles|quality)/[^`\s,)]+\.md)(?:#([^`\s,)]+))?`")
    checked = 0
    for match in link_re.finditer(text):
        ...
        if fragment and not anchor_matches(fragment, anchors_for(dest)[0]):
            issues.append(f"principles/standards-mapping.md: anchor 누락 `{rel}#{fragment}`")
    ...
    for label, start_re in owasp_sections.items():
        start = next((i for i, line in enumerate(lines) if re.match(start_re, line)), None)
        if start is None:
            issues.append(f"principles/standards-mapping.md: {label} 섹션 누락")
            continue
        end = next((i for i in range(start + 1, len(lines)) if re.match(r"^### 4-\d\.|^## 5\.", lines[i])), len(lines))
        section = "\n".join(lines[start:end])
        if not re.search(r"`security/[^`\s,)]+\.md", section):
            issues.append(f"principles/standards-mapping.md: {label} 섹션 security reference 누락")
    return checked
...
def check_ai_llm_security():
    ...
    for term in required_terms:
        if term not in text:
            issues.append(f"security/security-ai-model.md: LLM 체크리스트 문구 누락 `{term}`")
    refs_required = [
        "security-authz",
    ...
    if ref_hits < 3:
        issues.append(f"security/security-ai-model.md: LLM 보안 cross-reference {ref_hits}개 (<3)")

check_ai_llm_security()

print(f"patterns index rows={patterns_rows}, security index rows={security_rows}, quality index rows={quality_rows}, principles aliases={principles_alias_rows}, standards links={standards_links}")
...
# ─────────────────────────────────────────────
# CHECK 13: semantic index / standards mapping 검증
# ─────────────────────────────────────────────
echo ""
check_semantic_catalog
