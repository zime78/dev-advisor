...
#   [11-11] handoff.md 3 에이전트 + model="opus"
...
check_research_mode() {
  ...
  # 11-11. handoff.md 에 3 에이전트 + model="opus" 명시
  if [ -f "${handoff_file}" ]; then
    ...
    local opus_count
    opus_count=$(grep -c 'model="opus"' "${handoff_file}" 2>/dev/null || echo 0)
    if [ "${handoff_missing}" -eq 0 ] && [ "${opus_count}" -ge 1 ]; then
      ok "[11-11] handoff.md 3 에이전트 등록 + model=\"opus\" ${opus_count}회"
    elif [ "${handoff_missing}" -gt 0 ]; then
      fail "[11-11] handoff.md 누락 에이전트: ${handoff_missing}/3"
    else
      fail "[11-11] handoff.md model=\"opus\" 미명시"
    fi
  else
