...
### cross-skill 라우팅 — data-advisor (DMBOK)
...
| 발화 패턴 | 라우팅 | 예 |
|---------|--------|----|
| DMBOK 데이터 관리 키워드 (`MDM`/`마스터 데이터`/`골든 레코드`/`Survivorship`/`Match-Merge` (KA7) · `데이터 웨어하우스`/`DWH`/`Kimball`/`Star`/`Snowflake`/`SCD`/`OLAP`/`Lakehouse`/`dbt` (KA8) · `데이터 품질`/`DQ`/`Data Lineage`/`Data Catalog`/`Stewardship`/`OpenLineage` (KA10) · `CAP`/`PACELC`/`격리 수준`/`ACID`/`BASE`/`복제`/`파티셔닝`/`정규화` (KA2) · `B+Tree`/`LSM Tree`/`WAL`/`MVCC`/`Buffer Pool`/`쿼리 옵티마이저`/`EXPLAIN ANALYZE` (KA3) · `DMBOK`) | `data-advisor` 스킬 호출 | "MDM 골든 레코드 설계" → `/mdm` 또는 `/data <id>` |

- **[필수]** hand-off 호출은 `/data <id>` · `/data list` · `/data search <kw>` 로 한다. 단축 진입점 `/mdm`(KA7) · `/dwh`(KA8) · `/dq`(KA10) 도 동일 스킬로 매핑된다 (예: `/data mdm-golden-record`). `data-advisor` 는 Codex `spawn_agent` 서브에이전트가 아니라 별도 스킬이므로 `spawn_agent(...)` 로 위임하지 않는다.
- **[권장]** dev-advisor 카탈로그(`patterns/data-modeling.md` CAP 실무 패턴 · `patterns/data-access.md` OO 영속성 · `algorithms/spatial.md`·`algorithms/search-systems.md` 공간/검색 · `security/security-data-protection.md` 데이터 보안 · `patterns/mlops.md` ML/AI 파이프라인)와 겹치는 주제는 dev-advisor 에 유지하고, **순수 DMBOK 데이터 관리 어휘(KA2/3/7/8/10)** 만 `data-advisor` 로 넘긴다. (data-advisor `## Out-of-scope` 와 대칭)
...
| **`data-advisor`** | **별도 스킬 호출** | **DMBOK 데이터 관리 주제(KA2/3/7/8/10) 감지** | **`/data <id>` · `/mdm` · `/dwh` · `/dq` 로 조회** |

호출 형식 + 모델 정책은 AGENTS.md OMX/ULW 분석·설계 모델 정책 + `references/handoff.md` 참조. `data-advisor` 는 Codex `spawn_agent` 서브에이전트가 아니라 **별도 스킬**이므로 `spawn_agent(...)` 가 아닌 스킬 호출(`/data ...`)로 hand-off 한다.
