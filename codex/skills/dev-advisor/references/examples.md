...
`full` 모드와 동일한 5 모드를 단일 context 순차가 아닌 **5 Codex 서브에이전트 병렬 + 1 reviewer 통합** 으로 실행하는 심층 audit. 각 모드가 독립 context 를 가져 깊이가 더해지지만 OMX `ultrawork` 의존 + 토큰 비용 ↑.

```
User: /dev-advisor swarm packages/data/lib/src/repositories/booking_repository_impl.dart
```

**ULW 발사** (5 + 1 = 6 agent 병렬):
...
spawn_agent(agent_type="verifier", message="통합 우선순위 정렬 + cross-link 매트릭스") — 5 결과 reviewer
```
...
- **적용 조건**: 단일 모듈이 핵심 비즈니스 도메인 AND 변경 빈도 ≥ 월 5회 AND OMX ultrawork 및 Codex 서브에이전트 사용 가능
