...
`full` 모드와 동일한 7 기본 모드를 단일 context 순차가 아닌 **7 Codex 서브에이전트 병렬 + 1 reviewer 통합** 으로 실행하는 심층 audit. 각 모드가 독립 context 를 가져 깊이가 더해지지만 OMX `ultrawork` 의존 + 토큰 비용 ↑.
...
| 측면 | full | swarm |
|------|------|-------|
| 실행 방식 | 7 기본 모드 순차 (단일 context) | 7 기본 모드 병렬 Codex 서브에이전트 + 1 reviewer |
| 토큰 비용 | 7만~14만 | 8만~16만 |
| 시간 | 5분 (순차) | 1.5분 (병렬) |
