...
**목적**: 도메인 + 제약 조건을 받아 543 패턴 / 292 알고리즘 / 75 언어 / 106 보안 / 211 원칙 / 20 품질 + 20 부록 카탈로그에서 후보 3~5 개를 추출하고, 정량 score 와 trade-off 로 1 위 후보를 권장한다. score 는 1~5 정수.
...
### 7. Hand-off 후보
- HIGH ≥ 3 → 카탈로그 lookup / 코드 구현 → `executor` / 0건 → `researcher`
...
4. researcher 에이전트 위임

추천 다음 명령:
- `/dev-advisor research "<재정제된 검색어>"`
- `/pattern search <키워드>`
- `spawn_agent(agent_type="researcher", message="...")`
```
