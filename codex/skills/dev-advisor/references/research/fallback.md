...
4. 0건 확정 시에만 검색어 자동 재정제 1회 시도 후 `researcher` hand-off.
...
| **0-of-3 정상** | output-format.md §6.2 0건 대응 출력 + 검색어 재정제 제안 | 검색 결과 없음 출력 + `researcher` hand-off 안내 |
...
Codex web 도구 자체가 사용 불가하거나 네트워크 차단 환경에 대한 도구 레벨 fallback.

```
1차: Codex web 도구 (3 API 직접 호출 — §4.4.1) — 🆓 완전 무료
  ↓ 실패 조건: 도구 미지원 / TLS 오류 / rate limit 소진 / Codex web 도구 자체 오류
2차: terminal curl (응답 신뢰성 사용자 검증 필수) — 🆓 무료
  ↓ 실패 조건: 네트워크 차단 / curl 명령 미허용 / 3 API 모두 차단
3차: researcher 에이전트 위임 — 🆓 Codex 내장
```

### 6.1 1차: Codex web 도구

기본 동작. Codex web 도구로 3 API를 직접 호출한다 (§4.4.1 도구 매핑 참조).
...
### 6.2 2차: terminal curl

Codex web 도구가 사용 불가한 환경에서 terminal 도구로 `curl`을 직접 실행한다.
...
- 응답 신뢰성은 Codex web 도구와 동일하나, **사용자에게 curl 결과임을 명시**하고 검증을 권장한다.
- 출력 하단 footnote 추가: "2차 fallback (curl) — 사용자 검증 권장"

### 6.3 3차: researcher 에이전트 위임

1차·2차 모두 실패 시 (또는 3 API 모두 네트워크 차단 시) Codex `researcher` 에이전트에 위임한다.
...
- Codex에 등록된 `researcher` 역할의 기본 모델 정책을 따른다.
- 출력 상단 배너: "3차 fallback — researcher 에이전트 위임 (외부 API 직접 접근 불가)"
- researcher 결과는 §11.1 사전 차단 정책(식별자 정규식 검증)을 동일하게 적용한다.
...
[Codex web 도구 가용?]
   ↓ YES                   ↓ NO
[3 API Codex web 호출]  [터미널 curl 2차 fallback]
   ↓                        ↓
   ↓                   [curl 가능?]
   ↓                    ↓ YES         ↓ NO
   ↓               [curl 호출]   [researcher 3차]
   ↓                    ↓
[응답 결과 집계]
   ↓
...
[Fast-Fail: 검색어 자동 정제 1회 재시도]
   ↓
[여전히 0건?] → §6.2 "검색 결과 없음" 출력 + researcher hand-off
```
