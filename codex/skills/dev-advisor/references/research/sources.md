...
| `data[].venue` | 발표처 (컨퍼런스/학술지명) | 응답 원문 그대로. Codex 추측 금지 |
| `data[].citationCount` | 총 인용수 | 응답 원문 그대로. Codex 추정 금지 |
...
| `results[].cited_by_count` | 인용수 | 응답 원문 그대로. Codex 추정 금지 |
...
| **인용수** | S2 또는 OpenAlex 응답 값만 사용. Codex 추정 금지 |
...
| **Codex cutoff 지식 혼입 금지** | "선택/판정" 6필드 작성 시 raw API 응답만 사용 |
